...
import {useAddProjectDomainMutation,useUpdateProjectDomainMutation} from'../../../redux/services/Project/domain'
import {NullableParam } from '../../../types';

type infoDataType = {
	id: number;
	name: NullableParam<string>;
	domainName:NullableParam<string>;
};
...
interface AddList {
	...
	onSuccess?: () => void;
	editMode: boolean;
	domainData?: NullableParam<infoDataType>;
}
...
const AddList: FC<AddList> = ({
	...
	const[UpdateDomain]=useUpdateProjectDomainMutation(); 
	const[AddDomain]=useAddProjectDomainMutation()
	...
	const onSubmit = async (data: any) => {
		try {
			...
			if (err?.status == 400) {
				showAlert(
					'This Domain already Exist.','error'
				);
			} else {
	...
	const performAction = async (data: any) => {
		if (editMode) {
		return await UpdateDomain(data).unwrap();		
		} else {
		  return (await AddDomain(data).unwrap());
		}
	  };
